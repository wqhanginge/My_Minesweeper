...
void selectMapUnits(PGameInfo pGame, int index, int prev, bool area)
{
    ...
    if (isidxinmap(pGame, prev)) {
        Neighbor idxes;
        getNeighbors(pGame, idxes, index2x(pGame, prev), index2y(pGame, prev));
        for (int i = 0; i < NEI_TOTAL; i++) {
            if (idxes[i] != INV_INDEX) selectOne(pGame, idxes[i], false);
        }
    }

    //set new selected state
    if (isidxinmap(pGame, index)) {
        Neighbor idxes;
        getNeighbors(pGame, idxes, index2x(pGame, index), index2y(pGame, index));
        for (int i = 0; i < ((area) ? NEI_TOTAL : 1); i++) {
            if (idxes[i] != INV_INDEX) selectOne(pGame, idxes[i], true);
        }
    }
}
...
{
    RECT rcmap = { 0, 0, MAP_WIDTH(pGame->width), MAP_HEIGHT(pGame->height) };
    RECT rcmu = { -MUP_SIZE, -MUP_SIZE, MUP_SIZE * 2, MUP_SIZE * 2 };
    OffsetRect(&rcmu, index2px(pGame, index), index2py(pGame, index));
    IntersectRect(pclip, &rcmap, &rcmu);
}
...
void paintMapUnit(HDC hdc, int left, int top, BYTE mapunit)
{
    ...
    case MUS_COVER:
        ...
        break;
    case MUS_MARK:
        drawMapUnitMark(hdc, left, top, ISMUSELECTED(mapunit));
        break;
    case MUS_FLAG:
...
void paintMap(HDC hdc, int left, int top, PGameInfo pGame)
{
    for (int idx = 0; idx < pGame->width * pGame->height; idx++) {
        paintMapUnit(hdc, left + index2px(pGame, idx), top + index2py(pGame, idx), pGame->map[idx]);
    }
}
