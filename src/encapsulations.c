...
bool isLparamInRB(LPARAM lparam, BYTE num_map_width)
{
    POINTS p = MAKEPOINTS(lparam);
    p = (POINTS){ p.x - RB_LEFT(num_map_width), p.y - RB_TOP };
    return (p.x >= 0 && p.x < RB_SIZE && p.y >= 0 && p.y < RB_SIZE);
}

bool isLparamInMap(LPARAM lparam, BYTE num_map_width, BYTE num_map_height)
{
    POINTS p = MAKEPOINTS(lparam);
    p = (POINTS){ p.x - MAP_LEFT, p.y - MAP_TOP };
    return (p.x >= 0 && p.x < MAP_WIDTH(num_map_width) && p.y >= 0 && p.y < MAP_HEIGHT(num_map_height));
}
