...
bool lparamIsInRB(LPARAM lparam, BYTE num_map_width)
{
...
bool lparamIsInMap(LPARAM lparam, BYTE num_map_width, BYTE num_map_height)
{
    POINTS p = MAKEPOINTS(lparam);
    p = (POINTS){ p.x - MAP_LEFT, p.y - MAP_TOP };
    return (p.x >= 0 && p.x < x2px(num_map_width) && p.y >= 0 && p.y < y2py(num_map_height));
}
